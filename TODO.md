...
斷詞這件事 Sudachi 已經解決（連原形一起給）。缺的是**中文釋義**：

- **JMdict**（21 萬條目、31 萬 headword-reading 組合，CC-BY-SA）是日英字典的事實標準，
  Jisho.org、Yomitan 都用它。**JMnedict** 專門收人名地名 —— 正好對應假名標注錯最多的那類。
  **KANJIDIC2** 是漢字資料。`jmdict-yomitan` 有現成的整理格式。
- 問題是這些都是**日英**。開源的日中字典資源稀少。
- 兩條路：JMdict 的英文釋義再用 LLM 轉中文（可預先批次處理、存後端），
  或直接讓 LLM 產生釋義（彈性高但每次查都要打 API）。

- [ ] 決定走哪條路
- [ ] 字典要走後端不要綁進 App（照 TODO 既有原則，App 才能不用重裝就更新）
